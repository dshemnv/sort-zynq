...
# ----------------------------- Host app section ----------------------------- #
APPTARGET := $(filter benchmark yolo-app,$(MAKECMDGOALS))

ifeq ($(APPTARGET), benchmark)
MAIN_FILE := mainbenchmark.cpp
APP_NAME := sortzynqbenchmark
else ifeq ($(APPTARGET), yolo-app)
...
$(BUILD_DIR)/mainbenchmark.o: $(SRC_DIR)/mainbenchmark.cpp
	$(CXX) $(CXXFLAGS) -c $< -o $@
